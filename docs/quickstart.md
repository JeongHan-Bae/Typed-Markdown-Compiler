...
This repository turns Markdown content into a static Vue 3 website. Vue runs during the Node build through server-side rendering; the generated site contains HTML, CSS, copied public files, and browser assets, not a browser application runtime.
...
Normal Markdown links such as `https://...` and `mailto:...` remain external links. For a checked-in RSS file, put `feed.xml` under `public/` (`public/feed.xml`), not at the repository root; the build copies it to `dist/feed.xml`, and `/feed.xml` receives the configured repository base path during a GitHub Pages build. External schemes are limited to `http`, `https`, `mailto`, `tel`, and `ftp`; encoded, whitespace-obfuscated, or otherwise unrecognized schemes are rejected during compilation.
...
Content discovery recursively finds every published Markdown file below `content/`, so a nested file such as `blogs/series/first-step.md` always receives the logical route `blogs:series:first-step`. Collection mounting is separate: `source: blogs/` contains published documents whose direct parent directory is `blogs/`, excluding directory index heads. A deeper route is not automatically inserted into that parent list. It joins a nested collection when that exact directory has either an implicit head such as `blogs/series/index.md` or an external head that declares `type: list` with `source: blogs/series/`.
...
Place browser-renderable images under the configured asset directory. The default is `public/assets/`:
...
The repository includes a root `.env` file containing only five user-facing keys, all set to an empty string. The compiler parses it with the `dotenv` library. For these five keys only, a non-empty file value takes precedence over the same process variable. An empty file value does nothing, so the process variable and then the compiler's existing resolution/default behavior remain available.
...
ASSET_DIRECTORY="public/assets"
~~~

The root `.env` is loaded by the build, local launcher, Vite preview, and GitHub Pages build. The five-key priority above does not apply to protected pipeline values. `VITE_BASE_PATH`, repository name, GitHub identity, Host/Port, environment-file selectors, and runtime-test controls cannot be overridden by `.env`; adding those keys to the file has no effect. They retain their existing process, GitHub Actions, Git, command-line, and default sources.
...
ASSET_DIRECTORY=site-public/browser-assets \
npm run build
~~~

`PUBLIC_DIRECTORY` names the public-file root, such as `site-public`; `ASSET_DIRECTORY` defaults to its `assets/` child and can point to another child such as `site-public/browser-assets`. The configured asset directory is excluded from ordinary public-file copying and is copied separately to `dist/assets/`. Keep `PUBLIC_DIRECTORY` as the root, not as the asset directory. For example, `site-public/feed.xml` becomes `/feed.xml`, while `site-public/browser-assets/icons/marker.svg` is referenced as `asset:icons/marker.svg`. Other path prefixes such as `asset:public/feed.xml` do not address public-root files and are invalid.
...
Run the isolated full runtime test:

~~~bash
npm run rt-test
~~~

The runtime test uses disposable fixtures in dev/rt-test/fixtures/, checks every generated fixture page, its nested SVG asset, and the public-root `feed.xml`, exercises the documented syntax page, verifies that the draft fixture is not routable, and then stops Vite automatically. It does not read or modify the example content/.
